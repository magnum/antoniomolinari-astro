---
title: About
description: Antonio Molinari — developer and designer near Cremona, Italy.
  Founder of Incode, CTO and mentor, teaching web design at LABA Brescia since
  2013.
---
Hi, I'm **Antonio Molinari** — Magnum to friends. Developer and designer, I
live near Cremona in northern Italy with my wife and two kids. I've been
working on the web since the late 90s, so I've seen quite a few things come and
go. With my studio **Incode** I handle web development projects: mainly
**Ruby/Rails** (an almost irrational attachment, I'll admit), open source CMS,
Node.js and frontend development with Vue, AI integration, and everything in
between — from infrastructure to UX. I've also served as **CTO** and **mentor**
for several startups, on projects ranging from blockchain-based asset tracking
to document management. Since 2013 I've been teaching web design at **LABA —
Libera Accademia di Belle Arti** in Brescia, which keeps me in constant contact
with people approaching technology from a creative rather than a technical
background. Passionate about electronic music, live concerts, and anything with
a chip, a network connection, or something worth taking apart to understand how
it works.

[Resume available here](https://docs.google.com/document/d/1zAjYVGjGfPNKLzoo5UkYiG5BqNI_of1aLyQGxmn9Gvw)

In the past 20 years I collaborated with various agencies and worked for their
clients or directly with Axa, Bmw, Bnp Paribas, Cantine Ferrari, Cardif, Deutsch
Bank, Disney, Edison, Enel, FIA, Fastweb, Fiat/FCA, Gucci, Il Sole 24 ore, ING,
Intesa San Paolo, Loro Piana, Mailup, Manfrotto, Mediaworld, Mediolanum,
Nespresso, Nestlè, Negroni, Paypal, Perugina, Philadelfia, Piaggio, Pioneer,
Pirelli, Prosciutto di Parma, Rcs, Saint-Gobain, Sanpellegrino, Santander, Sky,
Sòl Melià, Tim, Twinings, Unicef, Unicredit, Vodafone and others.

For an updated portfolio, please visit the studio website at [incode.it](http://www.incode.it), or ask :-)

## GitHub Activity

[![Antonio Molinari's GitHub contributions over the last year](https://ghchart.rshah.org/01e0b8/magnum)](https://github.com/magnum)

[Follow me on GitHub →](https://github.com/magnum)

## Skills

Ruby/Rails\
Php cms oriented, WordPress, Drupal\
NodeJS\
Javascript, Vue, JAMstack\
Web Design and Development, Html/Css, front-end frameworks\
UX and Interface Design, Figma\
Git, CI, CD\
PostgreSQL, MySQL, SQLite, MsSQL\
Amazon Web Services EC2, LB, IAM, S3, RDS, QLDB, SQS, ElasticEncoding\
PaaS, Heroku, Kamal\
Server management, Docker, DevOps, metrics and resources management\
Blockchain, Bitcoin, Ethereum\
Python/Django\
MacOS, Linux Ubuntu/Debian/Centos, Windows\
Agile methodologies
